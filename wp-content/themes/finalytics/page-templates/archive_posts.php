<?php
/**
 *Template Name: Archive Blog posts
 */
get_header(); ?>

<article id="primary" class="archive-posts">
  <?php //get_template_part('template-parts/hero-small'); ?>

  <!-- Section Repeater -->
  <?php if ( have_rows('archive_posts_section_repeater') ) : ?>
    <?php while( have_rows('archive_posts_section_repeater') ): the_row(); ?>
      <!-- Modules Repeater -->
      <?php if( have_rows('archive_posts_flexible_modules') ):
        while ( have_rows('archive_posts_flexible_modules') ) : the_row();
