...
<section <?php if (!empty($hero_small_custom_id)): ?>id="<?php echo $hero_small_custom_id; ?>"<?php endif; ?>
         ...
         if (get_sub_field("hero_small_hide_overflow")): ?>overflow-hidden<?php endif; ?> <?php if (get_sub_field('hero_small_background_image') == 'waves'): ?>bg-waves<?php
         else: ?>bg-dotted<?php endif; ?> <?php if (get_sub_field('hero_small_banner_height') == 'medium'): ?>height-medium<?php
         elseif (get_sub_field('hero_small_banner_height') == 'small'): ?>height-small<?php
         elseif (get_sub_field('hero_small_banner_height') == 'tiny'): ?>height-tiny<?php endif; ?>">
  <div class="container-xl">
    <div class="hero-small_content">
      <?php if (!empty(get_sub_field("hero_small_eyebrow"))): ?>
        <span class="hero-small_eyebrow"><?php echo get_sub_field("hero_small_eyebrow"); ?></span>
      <?php endif; ?>
      <h1 class="h1-medium hero-small_title"><?php echo wp_trim_words( get_sub_field("hero_small_title"), 4, '' ); ?></h1>
      <?php if (!empty(get_sub_field("hero_small_text"))): ?>
        <p class="hero-small_text"><?php echo get_sub_field("hero_small_text"); ?></p>
      <?php endif; ?>
