...
function wptools_auto_update_plugins ( $update, $item ) {
	global $wptools_auto_updates;
	if ($wptools_auto_updates == 'no') {
				// use default settings
				return $update; 
	} elseif ($wptools_auto_updates == 'all') {
				// update plugin
				return true; 
	}
	$wptools_plugin_data = get_file_data(__FILE__, array('Name' => 'Plugin Name'), false);
    $wptools_plugin_name    = $wptools_plugin_data['Name'];
	if ( $wptools_plugin_name == 'wptools' and $wptools_auto_updates == 'yes')   {
		// update plugin
		return true; 
	} 
	else {
...
if($wptools_auto_updates != 'no')
  add_filter( 'auto_update_plugin', 'wptools_auto_update_plugins', 10, 2 );
